...
@export var drag_speed: float = 0.6  # menší = pomalejší tažení
...
func _unhandled_input(event):
	if event is InputEventMouseButton:
		...
		# Střední tlačítko: začátek/konec tažení
		...
		global_position = drag_start_camera_pos + delta * zoom * drag_speed
