...
func _on_ready() -> void:
	populate_tile_flags_from_tilemap(self)

func populate_tile_flags_from_tilemap(tilemap: TileMapLayer):
	for pos in tilemap.get_used_cells():
		var tile_data = tilemap.get_cell_tile_data(pos)
		if tile_data:
			Data.set_tile_flag(pos,"occupied",false)
			Data.set_tile_flag(pos,"hasAlien",false)
			Data.set_tile_flag(pos,"hasCrosshair",false)
			Data.set_tile_flag(pos,"house",false)
			Data.set_tile_flag(pos,"farm",false)
			Data.set_tile_flag(pos,"mine",false)
			Data.set_tile_flag(pos,"generator",false)
			Data.set_tile_flag(pos,"turret",false)
			
			var isMineral = tile_data.get_custom_data("IsMineral")
			Data.set_tile_flag(pos,"IsMineral",isMineral)
			
			var isCenter = tile_data.get_custom_data("IsCenter")
			Data.set_tile_flag(pos,"IsCenter",isCenter)
			
			var canBuild = tile_data.get_custom_data("CanBuild")
			Data.set_tile_flag(pos,"CanBuild",canBuild)
...
func place_target_marker(tile_pos: Vector2i) -> void:
	...
	# Find the original marker node (the one you want to duplicate)
